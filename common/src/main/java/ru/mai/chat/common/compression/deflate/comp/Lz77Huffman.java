...
public record Lz77Huffman(
    ...
    private static byte[] calcHuffmanCodeLengths(int[] symbolHistogram, int maxLen) {
        ...
        for (int i = 0; i < maxLen; i++) {
            nodes.addAll(leaves);
            Collections.sort(nodes, Comparator.comparingLong(Node::frequency));
    ...
    public Decision decide(byte[] b, int off, int historyLen, int dataLen) {
        return new Decision() {
            ...
            public void compressTo(BitOutputStream out, boolean isFinal) throws IOException {
                ...
                var symbolsAndExtraBits = ShortBuffer.allocate(Math.toIntExact(ceilDiv(dataLen * 4L, 3) + 1));
    ...
    private interface Node {
        ...
        void countOccurrences(byte[] nodeHistogram);
