...
import ru.mai.chat.common.cipher.FeistelCipherMARS;
...
import ru.mai.chat.common.round_keys.MARS.RoundKeyGenerationMARS;

public class MARS implements EncryptionAlgorithm {
    public static final int KEY_LENGTH_MARS_16 = 16;
    public static final int BLOCK_LENGTH_MARS = 16;
    // region Sbox
    ...
    private static final Logger log = LoggerFactory.getLogger(MARS.class);
    private final int roundCount = 16;
    private final FeistelCipherMARS cipher;
    // endregion

    public MARS(byte[] key) {
        if (key.length != 16) {
            throw new IllegalArgumentExceptionWithLog("MARS: Param key must be of size 16", log);
        }

        RoundKeyGenerationMARS roundKeyGenerationMARS = new RoundKeyGenerationMARS();
        this.cipher = new FeistelCipherMARS(roundKeyGenerationMARS, key);
    }


    @Override
    public byte[] encrypt(byte[] input) {
        return cipher.methodForConstructingBlockCiphersEncryption(input, roundCount);
    }

    @Override
    public byte[] decrypt(byte[] input) {
        return cipher.methodForConstructingBlockCiphersDecryption(input, roundCount);
    }

    @Override
    public int getAlgorithmBlockForEncryption() {
        return BLOCK_LENGTH_MARS;
    }
